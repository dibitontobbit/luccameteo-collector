...
async function main() {
  ...
  while (true) {
    ...
    for (const row of targets) {
      ...
      try {
        ...
        if (updated % 100 === 0) {
          console.log(`✓ ${updated} letture vento convertite`);
        }
      } catch (error) {
        failures++;
    ...
    // Le liste sono ordinate dalla più recente alla più vecchia.
    // Quando l'ultima riga della pagina è precedente a settembre, possiamo fermarci.
  ...
  console.log(
    `✓ Migrazione conclusa. Scansionate ${scanned}, candidate ${matched}, aggiornate ${updated}, errori ${failures}`
  );
